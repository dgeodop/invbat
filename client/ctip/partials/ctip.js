...
exports.nomEtabl = function(req, res) {
	var query = 'SELECT id_etabl, nom_court, ctip_nom_court, sap, nom_long FROM etabl WHERE util_ctip=1 ORDER BY nom_court';
	pg.connect(connectString, function(err, client, done) {
		if(err) { return console.error('Problème de connection à la base de données', err); }
		client.query(query, null, function(err, result) {
			done();
...
exports.nomRelatifEtabl = function(req, res) {
	var query = 'SELECT id_etabl, nom_court, ctip_nom_relatif, sap, nom_long FROM etabl WHERE util_ctip=1 ORDER BY nom_court';
	pg.connect(connectString, function(err, client, done) {
		if(err) { return console.error('Problème de connection à la base de données', err); }
		client.query(query, null, function(err, result) {
			done();
